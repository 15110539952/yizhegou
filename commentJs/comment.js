...
function countdowm(that,timestamp,is_task,data,i) {
  if (timestamp.length < 13) { timestamp = timestamp*1000}
  let timer = setInterval(function () {
    ...
    if (t > 0) {
      ...
      if (day < 0) {
        if (is_task) {
          data.task[i].Time = { myHour: hour < 10 ? "0" + hour : hour, myMinite: min < 10 ? "0" + min : min, mySecond: sec < 10 ? "0" + sec : sec };
          that.setData({
            activityDetail: data
          })
          return false;
